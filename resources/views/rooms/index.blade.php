<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div id="rooms-container" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7">
                @foreach ($rooms as $room)
                    <div class="room-card bg-white overflow-hidden shadow-sm sm:rounded-lg" data-state="{{ $room->state }}" data-category="{{ $room->category }}" data-price="{{ $room->price_per_night }}">
                        <img src="{{ $room->picture }}" alt="{{ $room->room_number }}" class="w-full h-72 object-cover">
                        <div class="p-4">
                            ...
                            <a href="{{ route('rooms.show', $room->id) }}" class="inline-block mt-2 bg-gray-800 hover:bg-gray-700 text-white py-2 px-4 rounded-md shadow-lg transition duration-300">
                                Ver detalles
                            </a>
                        </div>
                    </div>

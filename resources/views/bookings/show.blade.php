            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 flex justify-between">
                    <div class="flex-1">
                        ...
                        <div class="mt-16">
                            <a href="{{ route('bookings.edit', $booking->id) }}" class="inline-block bg-gray-800 hover:bg-gray-700 text-white py-2 px-24 rounded-md shadow-lg transition duration-300">
                                Editar
                            </a>
                            <form action="{{ route('bookings.destroy', $booking->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta reserva?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-block bg-red-900 hover:bg-red-700 text-white py-2 px-24 rounded-md shadow-lg transition duration-300">
                                    Eliminar
                                </button>
                            </form>
